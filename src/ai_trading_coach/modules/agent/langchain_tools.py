...
def _build_tool(
    ...
    return StructuredTool.from_function(
        ...
        name=action_name,
        description=(
            f"Fetch {evidence_type.value} evidence via MCP. "
            "Pass objective, tickers, query and time_window when available."
        ),
    )
...
async def _execute_tool_async(
    *,
    ...
    trace = ToolCallTrace(
        ...
        tool_name=tool_ref.tool_name,
        server_id=tool_ref.server_id,
        request_summary=f"react_tool={action_name}",
    ...
    return f"Fetched {len(items)} items from {tool_ref.key}. evidence_item_ids={[item.item_id for item in items]}"
