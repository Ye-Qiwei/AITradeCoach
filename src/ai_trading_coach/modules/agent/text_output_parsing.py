...
def parse_parser_output_text(raw_text: str) -> ParserOutput:
    ...
    for _, heading in _subsections(judgements_h1, "h2"):
        ...
        if category not in _ALLOWED_CATEGORIES or not target or not thesis:
            continue
        judgements.append(JudgementItem(category=category, target=target, thesis=thesis, evaluation_window=_normalize_window(fields.get("evaluation_window"))))
    return ParserOutput(trade_actions=trade_actions, judgements=judgements)
...
def parse_research_output_text(raw_text: str, *, judgements: list[JudgementItem]) -> list[dict[str, Any]]:
    ...
    if len(subsections) != len(judgements):
        raise ValueError(f"Research output subsection count mismatch: expected {len(judgements)}, got {len(subsections)}")
...
def parse_single_research_output_text(raw_text: str, *, fallback_judgement: JudgementItem) -> dict[str, Any]:
    parsed = parse_research_output_text(raw_text, judgements=[fallback_judgement])
    if len(parsed) != 1:
        raise ValueError(f"Expected single judgement research output, got {len(parsed)}")
    return parsed[0]


def build_research_output_from_items(items: list[ResearchedJudgementItem]) -> ResearchOutput:
    return ResearchOutput(judgements=items)


def collected_items_from_evidence(evidence_items: list[Any]) -> list[CollectedEvidenceItem]:
    return [
        CollectedEvidenceItem(
            evidence_type=str(getattr(e, "evidence_type", "other")),
            summary=str(getattr(e, "summary", "")),
            related_tickers=list(getattr(e, "related_tickers", [])),
            sources=[EvidenceSource(provider=str(getattr(src, "provider", "")), title=getattr(src, "title", None), uri=getattr(src, "uri", None), published_at=str(getattr(src, "published_at", "")) or None) for src in getattr(e, "sources", [])],
        )
        for e in evidence_items
    ]


def build_researched_judgement(*, fallback: JudgementItem, parsed: dict[str, Any], evidence_items: list[Any]) -> ResearchedJudgementItem:
    support_signal = str(parsed.get("support_signal", "uncertain")).lower()
    evidence_quality = str(parsed.get("evidence_quality", "insufficient")).lower()
    ...
    return ResearchedJudgementItem(
        ...
        evidence=JudgementEvidence(
            ...
            collected_evidence_items=collected_items_from_evidence(evidence_items),
        ),
    )
...
def parse_reporter_output_text(raw_text: str, judgement_count: int) -> ReporterOutput:
    ...
    for row in rows:
        ...
        feedback.append(DailyJudgementFeedback(initial_feedback=initial_feedback, evaluation_window=_normalize_window(data.get("evaluation_window"))))
